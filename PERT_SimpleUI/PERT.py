...
# Parse the problem from the provided string
def parse_problem(problem):
    ...
    for line in problem.splitlines():
        ...
        if(len(line) > 0 and line.strip() != '' and line[0] != '!'):
            ...
            if(len(tokens) < 3):
                return('Error on line ' + str(lineno) + ': Invalid activity definition \"' + line.strip() + '\"')
                
            # Check that the duration is a valid number
            if(not tokens[1].isnumeric()):
                return('Error on line ' + str(lineno) + ': Invalid activity duration \"' + tokens[1] + '\"') 
            elif(int(tokens[1]) < 0):
                return('Error on line ' + str(lineno) + ': Activity duration less than 0')
            
            # Check that the required labour is a valid number
            if(not tokens[2].isnumeric()):
                return('Error on line ' + str(lineno) + ': Invalid labour requirement \"' + tokens[2] + '\"')
            elif(int(tokens[2]) < 0):
                return('Error on line ' + str(lineno) + ': Labour requirement less than 0')
            
            # Check that activity does not exist already
            for activity in activities:
                if(activity.name == tokens[0]):
                    return('Error on line ' + str(lineno) + ': Activity redefined \"' + line.strip() + '\"')
            ...
            for i in range(3, len(tokens)):
                ...
                if(not found):
                    return('Error on line ' + str(lineno) + ': Prerequisite undefined \"' + tokens[i] + '\"')
